...
describe('Maven Scope Integration Tests', () => {
  ...
  describe('with sbomMavenScopeProperties enabled', () => {
    ...
    test('should handle compile-only dependencies', async () => {
      ...
      dependencyNodes.forEach(node => {
        expect(node.info?.labels?.['build:scope']).toBe('maven:compile');
      });
    });
  });
  ...
  describe('verbose mode tests', () => {
    describe('with sbomMavenScopeProperties enabled and verbose mode', () => {
      const baseOptions: MavenOptions = {
        dev: false,
        scanAllUnmanaged: false,
        'print-graph': true,
    ...
    describe('with sbomMavenScopeProperties disabled and verbose mode', () => {
      const baseOptions: MavenOptions = {
        dev: false,
        scanAllUnmanaged: false,
        'print-graph': true,
